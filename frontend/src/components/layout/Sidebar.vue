  ...
  <aside :class="['bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-700 h-screen flex-col hidden md:flex shrink-0 transition-all duration-300', isCollapsed ? 'w-20' : 'w-64']">
    ...
    <nav class="flex-1 p-4 space-y-2 overflow-y-auto overflow-x-hidden" :class="isCollapsed ? 'px-2' : ''">
      <div v-if="!isCollapsed" class="px-3 py-2 text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-2">
        {{ userRole === 'Worker' ? 'WORKER MENU' : 'HSE DEPARTMENT' }}
      </div>
      ...
      <router-link :to="userRole === 'Worker' ? '/hse' : '/hse'" active-class="bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-100" :class="['flex items-center gap-3 py-2.5 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-slate-100 rounded-md font-medium transition-colors', isCollapsed ? 'justify-center px-0' : 'px-3']" :title="isCollapsed ? (userRole === 'Worker' ? 'My Permits' : 'HSE Command Center') : ''">
        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-[var(--color-saipem-tertiary)] shrink-0">
          <path v-if="userRole === 'Worker'" d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline v-if="userRole === 'Worker'" points="14 2 14 8 20 8"/><line v-if="userRole === 'Worker'" x1="16" x2="8" y1="13" y2="13"/><line v-if="userRole === 'Worker'" x1="16" x2="8" y1="17" y2="17"/><polyline v-if="userRole === 'Worker'" points="10 9 9 9 8 9"/>
          <path v-if="userRole !== 'Worker'" d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z"/><path v-if="userRole !== 'Worker'" d="M12 9v4"/><path v-if="userRole !== 'Worker'" d="M12 17h.01"/>
        </svg>
        <span v-if="!isCollapsed">{{ userRole === 'Worker' ? 'My Permits' : 'HSE Command Center' }}</span>
      </router-link>
      ...
      <!-- Incidents menu - All users can access -->
      ...
      <!-- Deck Location - Admin and Safety Officer only -->
      <router-link v-if="userRole === 'Admin' || userRole === 'Safety Officer'" to="/offshore/locations" active-class="bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-100" :class="['flex items-center gap-3 py-2.5 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-slate-100 rounded-md font-medium transition-colors', isCollapsed ? 'justify-center px-0' : 'px-3']" :title="isCollapsed ? 'Deck Location' : ''">
      ...
      <!-- Analytics - Admin and Safety Officer only -->
      ...
      <div v-if="userRole === 'Admin'" class="pt-4 mt-4 border-t border-slate-200 dark:border-slate-700">
        <div v-if="!isCollapsed" class="px-3 py-2 text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-2">
          MASTER DATA
        </div>
        <div v-else class="h-4 mb-2"></div>
      ...
      <div v-if="hasAssetAccess" class="pt-4 mt-4 border-t border-slate-200 dark:border-slate-700">
        ...
        <!-- Assets -->
        <router-link to="/assets" active-class="bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-100" :class="['flex items-center gap-3 py-2.5 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-slate-100 rounded-md font-medium transition-colors', isCollapsed ? 'justify-center px-0' : 'px-3']" :title="isCollapsed ? 'Asset Management' : ''">
          <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="shrink-0"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="12" x2="12" y1="11" y2="17"/><line x1="9" x2="15" y1="14" y2="14"/></svg>
          <span v-if="!isCollapsed">Asset Management</span>
        </router-link>
      </div>
    </nav>
  </aside>
