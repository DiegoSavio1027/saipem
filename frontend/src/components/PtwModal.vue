                  ...
                  <PopoverContent class="p-0 w-[550px]">
                      <Command>
                          ...
                          <CommandList>
                              ...
                              <CommandGroup>
                                  <CommandItem
                                      v-for="emp in availableWorkersForCrew"
                                      :key="emp.emp_id"
                                      :value="emp.emp_id"
...
const availableWorkersForCrew = computed(() => {
    return availableUsers.value.filter(emp => emp.job_role === 'Worker');
});
...
watch(selectedWorkOrder, (newWoId) => {
    if (newWoId) {
        ...
        if (selectedWO) {
            // Auto-fill location
            if (selectedWO.asset_assigned_decks && selectedWO.asset_assigned_decks.length > 0) {
                formData.value.deck_location = String(selectedWO.asset_assigned_decks[0].id);
            }
            // Auto-fill crew from assigned_to
            if (selectedWO.assigned_to && !selectedCrew.value.includes(selectedWO.assigned_to)) {
                selectedCrew.value = [selectedWO.assigned_to];
            }
        }
    }
});
